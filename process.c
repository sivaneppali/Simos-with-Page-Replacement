...
{  
  // *** ADD CODE to switch out the context from CPU to PCB
	  ...
	  PCB[pid]->timeUsed = PCB[pid]->timeUsed+CPU.numCycles-intime; //Have to Check Again....
}
...
{ int pid, ret, i;
  ...
  { pid = new_PCB ();
    if (pid > idlePid)
    { ret = load_process (pid, fname);   // return #pages loaded into main memory
      if (ret > 0)
      { PCB[pid]->PC = 0;
        PCB[pid]->AC = 0;
...
{ int pid, intime;
  ...
  { 
	...
	context_in (pid);
	if(PCB[pid]->timeUsed==0){intime=CPU.numCycles-1;} //Calculating from that cycle, so taking one number less to calculate the 
	else{intime=CPU.numCycles;} 
    ...
    if (CPU.exeStatus == eReady) 
      ...
      { 
	//if (CPU.exeStatus == ePFault){PCB[pid]->numPF +=1;}   //Have to Check This.....
       	context_out (pid,intime);
